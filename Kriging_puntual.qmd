...
Kriging es una técnica de interpolación espacial que permite predecir valores desconocidos en una superficie a partir de los valores conocidos en ubicaciones cercanas. Es especialmente útil para datos de alta densidad y para datos con estructuras espaciales complejas. En este cuaderno, vamos a explorar los diferentes tipos de Kriging y cómo implementarlos en R.
...
Kriging es un método de predicción espacial para variables que varían continuamente en el espacio. La idea fundamental detrás de Kriging es que la predicción de un valor desconocido en una ubicación dada se hace como una combinación lineal ponderada de los valores observados en las ubicaciones vecinas. Los pesos de esta combinación se determinan utilizando la información de la covarianza entre el lugar a predecir y las ubicaciones observadas en el espacio. Esta técnica se utiliza comúnmente en estadística espacial, geología, minería, geofísica, oceanografía y otros campos que involucran la toma de medidas en lugares dispersos en el espacio.
...
donde las ponderaciones $λ_i$ se obtienen resolviendo el sistema de ecuaciones:

$$∑_{j=1}^n λ_j C(s_i,s_j) - C(s_i,s_0) = 0, \  i = 1,...,n.$$

El kriging simple se encuentra minimizando la varianza del error de predicción, esto es, la varianza dela diferencia entre el predictor y el valor verdadero de la variable y que está dado por:

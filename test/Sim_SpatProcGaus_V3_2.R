...
simSpatFunData <- function(argvals, Ph=FALSE, ValPh, M, eFunType, ignoreDeg = NULL, trueVals, scores, N)
...

{
    ...
    if (p == 1) 
    {
      ...
      else #Ph=TRUE. CURVES WITH PHASE. Each curve has its domain given by argvals
      {
         for (j in 1:N) 
         {
           trueFuns <- eFun(argvals = argvals[[1]]+ValPh[j], M = M, ignoreDeg = ignoreDeg, type = eFunType)
           if (j==1)
           {
